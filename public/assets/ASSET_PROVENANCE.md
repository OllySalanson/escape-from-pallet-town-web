...
- **Contents.** `potion`, `super-potion`, `antidote`, `poke-ball`, `great-ball`,
  ...
  `linen-roll`, and the one evolution stone `thunder-stone`. The materials are
  drawn in code by `scripts/draw-material-icons.mjs` (character art and small
  shapes on the same palette, written out as PNG), so they are original to this
  repository and can be regenerated byte for byte. `thunder-stone` is drawn the
  same way by `scripts/draw-evolution-stone-icons.mjs`: a cut stone lit from the
